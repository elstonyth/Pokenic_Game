...
export const metadata: Metadata = {
  ...
  description: "Redeem vouchers for free pulls and guaranteed buybacks on the claw machine.",
};
...
export default function VouchersPage() {
  return (
    <div className="w-full px-fluid py-10">
      ...
      <section className="relative mb-8 overflow-hidden rounded-2xl border border-white/10 bg-neutral-950">
        <div className="pointer-events-none absolute inset-0 flex">
          {HERO_SLABS.map((src, i) => (
            ...
            <img key={i} src={src} alt="" aria-hidden="true" className="h-full flex-1 object-cover opacity-25 blur-3xl saturate-150" />
          ))}
        </div>
        ...
        <div className="relative flex flex-col items-center px-6 py-12 text-center sm:py-14">
          ...
          <Reveal as="h1" className="font-heading text-3xl font-bold tracking-tight text-white sm:text-4xl">
            Your Vouchers
          </Reveal>
          <Reveal as="p" delay={80} className="mt-3 max-w-md text-sm leading-relaxed text-white/60 sm:text-base">
      ...
      <section>
        <h2 className="font-heading text-xl font-bold tracking-tight text-white">Active Vouchers</h2>
        <p className="mt-1 text-sm text-white/50">Redeem these for free pulls</p>
        <div className="mt-5 flex flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/[0.02] px-6 py-16 text-center">
          ...
          <p className="mt-1.5 max-w-sm text-[13px] leading-relaxed text-white/45">
            You don&apos;t have any active vouchers at the moment. Active vouchers will appear here when you receive them.
          </p>
        </div>
      </section>
    </div>
