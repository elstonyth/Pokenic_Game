...
const EXPIRES = ["expires in 1h", "expires in 1d", "expires in 2d", "expires in 3d", "expires in 5d"];
...
export default function BorrowLendPage() {
  ...
  const offers = [...OFFERS].sort((a, b) => (aprDir === "high" ? b.apr - a.apr : a.apr - b.apr));
    ...
    <div className="w-full px-fluid py-6">
      ...
      <div className="mb-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5">
          ...
          <h1 className="font-heading text-2xl font-bold tracking-tight text-white sm:text-3xl">Borrow / Lend</h1>
        </div>
        <button
          type="button"
      ...
      <div className="mb-5 grid grid-cols-2 gap-3">
        <button
          ...
          className={cn(
            ...
            mode === "lend" ? "border-white/25 bg-white/[0.07]" : "border-white/10 bg-white/[0.02] hover:bg-white/[0.04]",
          )}
        >
          <div className="text-sm font-semibold text-white">I want to lend USD</div>
          <div className="mt-1 text-xs text-white/45">Fund borrow offers and earn APR</div>
        </button>
        <button
          type="button"
          ...
          className={cn(
            "rounded-2xl border p-4 text-left transition-colors",
            mode === "borrow" ? "border-white/25 bg-white/[0.07]" : "border-white/10 bg-white/[0.02] hover:bg-white/[0.04]",
          )}
        >
          <div className="text-sm font-semibold text-white">I want to borrow USDC</div>
          <div className="mt-1 text-xs text-white/45">Use an item as collateral</div>
        </button>
      </div>
      ...
      <div className="mb-2 flex flex-wrap items-center gap-3">
        <div className="relative">
          <select value={collection} onChange={(e) => setCollection(e.target.value)} className={selectClass} aria-label="Filter by collection">
          ...
          <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/50" aria-hidden />
        </div>
        <div className="relative">
          <select value={aprDir} onChange={(e) => setAprDir(e.target.value as "high" | "low")} className={selectClass} aria-label="Sort by APR">
            <option value="high">APR: high to low</option>
            <option value="low">APR: low to high</option>
          </select>
          <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/50" aria-hidden />
        </div>
        <span className="ml-auto text-[13px] text-white/45">{OFFERS.length} active offers</span>
      </div>
          ...
          <Reveal as="div" key={o.id} delay={Math.min(i, 10) * 25}>
            <div className="flex flex-col gap-3 border-b border-white/[0.06] py-4 sm:flex-row sm:items-center sm:gap-4">
              ...
              <div className="flex min-w-0 flex-1 items-center gap-3">
                ...
                <img src={o.image} alt="" className="h-14 w-11 shrink-0 rounded bg-white/5 object-contain" />
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-white">{o.name}</p>
                  <p className="mt-0.5 truncate text-xs text-white/45">by @{o.user} · {o.expires}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-4 sm:justify-end">
                <div className="text-left sm:text-right">
                  <div className="text-sm font-semibold text-emerald-400">{usd(o.amount)}</div>
                  <div className="mt-1 flex items-center gap-1.5">
                    <span className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] font-medium text-white/70">{o.apr.toFixed(2)}% APR</span>
                    <span className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] font-medium text-white/55">{o.duration}</span>
                  </div>
                </div>
                <button
                  type="button"
