...
const JOBS = [
  { base: "elite-one-piece-pack", file: "elite-one-piece-pack-machine.avif", zones: [
    { kind: "url",   erase: [556, 860, 714, 904], draw: { text: "Pokenic.com", align: "center", x: 635, baseline: 895, fs: 30, color: "rgb(245,245,245)" } },
    { kind: "badge", erase: [704, 812, 830, 854], draw: { text: "by pokenic", align: "center", x: 767, baseline: 838, fs: 15, color: "rgb(40,28,22)", rot: 0 } },
  ] },
  { base: "legend-one-piece-pack", file: "legend-one-piece-pack-machine.avif", zones: [
    { kind: "url",   erase: [452, 864, 694, 908], draw: { text: "Pokenic.com", align: "center", x: 573, baseline: 898, fs: 28, color: "rgb(248,240,214)" } },
    { kind: "badge", erase: [686, 848, 858, 884], draw: { text: "by pokenic", align: "center", x: 772, baseline: 873, fs: 14, color: "rgb(48,38,20)", rot: 0 } },
  ] },
  { base: "starter-one-piece-pack", file: "starter-one-piece-pack-machine.avif", zones: [
    { kind: "url",   erase: [470, 872, 706, 916], draw: { text: "Pokenic.com", align: "center", x: 588, baseline: 906, fs: 29, color: "rgb(248,246,248)" } },
    { kind: "badge", erase: [700, 832, 826, 860], draw: { text: "by pokenic", align: "center", x: 763, baseline: 852, fs: 14, color: "rgb(60,40,48)", rot: 0 } },
  ] },
  // Yu-Gi-Oh (1440x900): placard "phygitals/claw." line 1 only, url, and the RIP&REVEAL pill.
  { base: "yugioh-pro-pack", file: "yugioh-pro-pack-machine.avif", zones: [
    { kind: "placard", erase: [548, 718, 700, 744], solid: true, draw: { text: "pokenic", align: "left", x: 552, baseline: 738, fs: 17, color: "rgb(235,110,75)" } },
    { kind: "url",     erase: [506, 838, 676, 868], draw: { text: "Pokenic.com", align: "center", x: 590, baseline: 862, fs: 20, color: "rgb(245,250,245)" } },
    { kind: "badge",   erase: [740, 804, 876, 828], draw: { text: "by pokenic", align: "center", x: 808, baseline: 821, fs: 14, color: "rgb(40,40,38)", rot: 0 } },
  ] },
];
...
function Image_dataurl(buf) { return `data:image/avif;base64,${buf.toString("base64")}`; }
...
await page.evaluate(async () => { await document.fonts.ready; const a = await document.fonts.load("700 40px Poppins"); const b = await document.fonts.load("600 24px Poppins"); if (!a.length || !b.length) throw new Error("Poppins failed to load"); });

const results = await page.evaluate(async ({ JOBS, inputs }) => {
  ...
  for (const j of JOBS) {
    ...
    const mc = mk.getContext("2d"); mc.fillStyle = "#000"; mc.fillRect(0, 0, W, H);
    ...
    for (const z of j.zones) {
      ...
      if (z.solid) {
        ...
        let r = 0, g = 0, b = 0, c = 0;
        for (let k = 0; k < SMP; k++) for (let xx = 0; xx < bw; xx++) { const i = (k * bw + xx) * 4; r += px[i]; g += px[i + 1]; b += px[i + 2]; c++; }
        r /= c; g /= c; b /= c;
        ...
        for (let xx = 0; xx < bw; xx++) {
          ...
          for (let k = 0; k < SMP; k++) {
            const i = (k * bw + xx) * 4; tr += px[i]; tg += px[i + 1]; tb += px[i + 2];
            const j = ((bh - 1 - k) * bw + xx) * 4; br += px[j]; bg += px[j + 1]; bb += px[j + 2];
          }
          tr /= SMP; tg /= SMP; tb /= SMP; br /= SMP; bg /= SMP; bb /= SMP;
          for (let yy = 0; yy < bh; yy++) {
            const t = bh > 1 ? yy / (bh - 1) : 0; const i = (yy * bw + xx) * 4;
            px[i] = tr + (br - tr) * t; px[i + 1] = tg + (bg - tg) * t; px[i + 2] = tb + (bb - tb) * t;
          }
        }
      }
      ...
      if (z.draw) {
        ...
        ctx.textAlign = dr.align; ctx.textBaseline = "alphabetic"; ctx.fillStyle = dr.color;
        if (dr.rot) { ctx.translate(dr.x, dr.baseline); ctx.rotate(dr.rot * Math.PI / 180); ctx.fillText(dr.text, 0, 0); }
        else ctx.fillText(dr.text, dr.x, dr.baseline);
        ctx.restore();
      }
      // freeze every rebranded zone in the bottom mask
      mc.fillStyle = "#fff"; mc.fillRect(x0, y0, bw, bh);
    }
    out[j.base] = { webp: cv.toDataURL("image/webp", 0.95), mask: mk.toDataURL("image/png") };
  }
  return out;
}, { JOBS, inputs });
...
for (const [base, r] of Object.entries(results)) {
  ...
  n++; console.log(`${base}: rebranded ${JOBS.find(j => j.base === base).zones.map(z => z.kind).join("+")} + mask`);
}
