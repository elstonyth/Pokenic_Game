...
if (!slugs.length) { console.error("usage: node scripts/verify-claw-anim.mjs <slug...>"); process.exit(1); }

const browser = await chromium.launch();
for (const slug of slugs) {
  const ctx = await browser.newContext({ viewport: { width: 1600, height: 1200 }, deviceScaleFactor: 1.5 });
  ...
  await img.screenshot({ path: `${OUT}/verify_${tag}.png` }).catch(async () => { await page.screenshot({ path: `${OUT}/verify_${tag}.png` }); });
  console.log(`${slug}: src=${src} box=${box ? Math.round(box.width) + "x" + Math.round(box.height) : "none"}`);
  await ctx.close();
}
await browser.close();
