...
async function cropOne(file) {
  ...
  for (let y = mid, run = 0; y < H; y++) {
    run = rowVal[y] <= BRIGHT ? run + 1 : 0;
    if (run >= RUN) { bottom = y - RUN; break; }
  }
  // No dark gap below the slab = no pedestal (already a slab-only scan, e.g. on
  // a white background) — leave the file untouched.
  if (bottom < 0) return { file: path.basename(file), skip: "already slab-only (no pedestal gap)" };
  if (top < 0 || bottom <= top) return { file, error: `no slab block (top=${top} bottom=${bottom})` };
  ...
  for (let x = 0, run = 0; x < W; x++) {
    ...
    if (run >= RUN) { left = x - RUN + 1; break; }
  }
  for (let x = W - 1, run = 0; x >= 0; x--) {
    run = colVal[x] > BRIGHT ? run + 1 : 0;
    if (run >= RUN) { right = x + RUN - 1; break; }
  }
  ...
  if (box.width > W * 0.95 && box.height > H * 0.95) {
    return { file: path.basename(file), skip: "already slab-only (full-frame box)" };
  }
  ...
  return { file: path.basename(file), box: `${box.width}x${box.height}@${box.left},${box.top}`, ratio: +ratio.toFixed(2) };
}

const files = fs.readdirSync(DIR).filter((f) => f.endsWith(".webp")).map((f) => path.join(DIR, f));
let errors = 0, outliers = 0, skipped = 0;
for (const f of files) {
  const r = await cropOne(f);
  if (r.error) { console.log(`ERROR  ${path.basename(f)}: ${r.error}`); errors++; continue; }
  if (r.skip) { console.log(`skip   ${r.file}  (${r.skip})`); skipped++; continue; }
  // a graded slab is ~1.4-1.8 tall:wide — flag anything else for eyeballing
  const flag = r.ratio < 1.3 || r.ratio > 1.95 ? "  <-- RATIO OUTLIER, inspect" : "";
  if (flag) outliers++;
  console.log(`${DRY ? "would crop" : "cropped"}  ${r.file}  -> ${r.box}  ratio ${r.ratio}${flag}`);
}
