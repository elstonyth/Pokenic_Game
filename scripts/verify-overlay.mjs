...
const SLUGS = ["pokemon-mythic", "nba-legend", "soccer-pro", "riftbound-starter", "nba-black"];
...
for (const slug of SLUGS) {
  ...
  if (!box) { console.log(`${slug}: NO machine img`); continue; }
  await page.screenshot({ path: `${OUT}/ov_${slug}.png`, clip: box });
  // zoom the placard zone (≈ x 30–55%, y 70–95% of the machine box)
  const z = { x: box.x + 0.30 * box.width, y: box.y + 0.70 * box.height, width: 0.30 * box.width, height: 0.26 * box.height };
  await page.screenshot({ path: `${OUT}/ov_${slug}_placard.png`, clip: z });
  console.log(`${slug}: src=${src?.split("/").pop()}  box=${Math.round(box.width)}x${Math.round(box.height)}`);
}
await browser.close();
console.log("done");
