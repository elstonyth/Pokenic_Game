...
const results = await page.evaluate(
  async ({ jobs, imgs, OVERRIDES, ASC, STEP }) => {
    const load = (s) =>
      new Promise((ok, no) => {
        const im = new Image();
        im.onload = () => ok(im);
    ...
    for (const job of jobs) {
      ...
      for (const key of ['placard', 'url']) {
        ...
        const band = ov.band || el.band,
          dir = el.dir;
        const bx0 = Math.round(band[0] * W),
          bx1 = Math.round(band[1] * W),
        ...
        for (let y = by0; y <= by1; y++)
          for (let x = bx0; x <= bx1; x++) {
            const p = (y * W + x) * 4,
              s = od[p] + od[p + 1] + od[p + 2];
            if (isHit(s)) hits.push([x, y, od[p], od[p + 1], od[p + 2], s]);
          }
        ...
        if (ov.pin) {
          ...
          for (let i = topIdx; i < rowc.length; i++) {
            if (rowc[i] >= rth) {
              botIdx = i;
              gap = 0;
            } else if (++gap > gapTol) break;
          }
          const topRow = by0 + topIdx,
            botRow = by0 + botIdx;
          anchorY = topRow;
          let lx = 1e9,
            rx = -1;
          for (const [x, y] of hits)
            if (y >= topRow && y <= botRow) {
              if (x < lx) lx = x;
              if (x > rx) rx = x;
            }
          anchorX = lx;
          ...
          // undetectable (white-on-white url) or forced: hardcode
          detected = false;
          const fb = ov.fallback ||
            el.fallback || { x: band[0], baseY: (band[2] + band[3]) / 2 };
          anchorX = Math.round(fb.x * W);
        ...
        for (let gy = ey0; gy <= ey1; gy++)
          for (let gx = ex0; gx <= ex1; gx++) {
            ...
            let lx = gx - 1;
            while (lx >= 0 && masked(lx, gy)) lx--;
            let rx = gx + 1;
            while (rx < W && masked(rx, gy)) rx++;
            let src = -1;
            if (lx >= 0 && rx < W) src = gx - lx <= rx - gx ? lx : rx;
            else if (lx >= 0) src = lx;
            else if (rx < W) src = rx;
            else continue;
            const sp = (gy * W + src) * 4,
...
await writeFile(
  OVERLAY_JSON,
  JSON.stringify({ ...existing, ...overlay }, null, 2),
);
await browser.close();
console.log(
