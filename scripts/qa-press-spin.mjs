...
// try/finally so a mid-run throw (navigation timeout, guarded read) still
// closes the browser instead of leaking a headless Chromium.
try {
  ...
  // sub-pixel tolerance: "at rest" must not flake on a rounding wobble
  if (s1.filter === 'none' && Math.abs(s1.x - s0.x) < 0.5)
    ok(`landed sharp and at rest (x=${s1.x.toFixed(1)})`);
  else
    fail(`still moving/blurred after settle: dx=${s1.x - s0.x}, ${s1.filter}`);
  ...
  if (centering < 3)
    ...
    fail(
      `nothing centered on the line (best off by ${centering.toFixed(1)}px)`,
    );
  ...
  if (periodic === 0)
    ok(
      `runway is non-periodic across ${runway.length} cells (randomized route)`,
    );
