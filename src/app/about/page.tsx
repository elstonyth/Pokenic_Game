...
const STEPS: Step[] = [
  { icon: ShoppingBag, title: "Buy a Pack", body: "Purchase digitally with card or crypto. Real physical cards from our inventory." },
  { icon: Sparkles, title: "Instant Reveal", body: "Watch your cards revealed live. Know exactly what you pulled." },
  { icon: Vault, title: "Securely Vaulted", body: "Cards stored in top-tier insured US facilities." },
  { icon: Truck, title: "Ship or Sell", body: "Redeem anytime with worldwide shipping, or sell back at 85%." },
];

type Feature = { icon: LucideIcon; stat: string; label: string; body: string };
const FEATURES: Feature[] = [
  { icon: Boxes, stat: "5+", label: "Card categories", body: "Sports, Pokemon, TCG, and more. One platform for every type of collectible card." },
  { icon: Layers, stat: "100K+", label: "Cards tokenized", body: "Own digital representations backed by real cards. Best of both worlds combined." },
  { icon: Zap, stat: "85%", label: "Buyback rate", body: "85% buyback guarantee on every card. Sell instantly without waiting for buyers." },
  { icon: Globe, stat: "24/7", label: "Always open", body: "24/7 marketplace accessible worldwide. Trade across borders without shipping delays." },
];

type VaultCard = { icon: LucideIcon; title: string; body: string };
const VAULT_CARDS: VaultCard[] = [
  { icon: Vault, title: "Choose Your Vault", body: "PSA, Alt, or Fanatics facilities" },
  { icon: ShieldCheck, title: "Fully Insured", body: "Complete coverage for all items" },
  { icon: Thermometer, title: "Climate Controlled", body: "Optimal storage conditions" },
];
    ...
    <div className="mx-auto w-full px-fluid py-4">
      ...
      <section className="relative mb-16 overflow-hidden rounded-2xl border border-white/10 bg-neutral-950">
        ...
        <div className="relative px-6 py-16 text-center sm:px-10 sm:py-20 lg:py-24">
          ...
          <Reveal as="p" delay={90} className="mx-auto mt-5 max-w-xl text-sm leading-relaxed text-white/65 sm:text-base">
      ...
      <section className="mb-16">
        <Reveal>
          <SectionHeading title="How It Works" sub="From purchase to ownership in four simple steps." />
        </Reveal>
            ...
            return (
              <Reveal key={s.title} delay={i * 90} className="h-full">
                <div className="flex h-full flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition-colors duration-300 hover:border-white/20">
                  ...
                  <h3 className="mb-2 font-heading text-sm font-semibold text-white">{s.title}</h3>
      ...
      <section className="mb-16">
        <Reveal>
          <SectionHeading title="Platform Features" sub="Everything you need to collect, trade, and grow." />
        </Reveal>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f, i) => {
            const Icon = f.icon;
            return (
              <Reveal key={f.label} delay={i * 90} className="h-full">
                <div className="flex h-full flex-col rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.02] p-6 transition-colors duration-300 hover:border-white/20">
                  ...
                  <div className="font-heading text-3xl font-bold text-white">{f.stat}</div>
                  <div className="mb-2 mt-1 text-[11px] uppercase tracking-wide text-white/45">{f.label}</div>
                  <p className="text-[13px] leading-relaxed text-white/55">{f.body}</p>
                </div>
              </Reveal>
            );
          })}
      ...
      <Reveal as="section" className="mb-16 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-transparent p-8 sm:p-10">
        <div className="grid gap-8 md:grid-cols-2 md:items-center">
          <div>
            <p className="mb-2 text-[11px] font-medium uppercase tracking-widest text-white/40">For Brands</p>
            <h2 className="font-heading text-2xl font-bold tracking-tight text-white sm:text-3xl">Launch With Us</h2>
            <p className="mt-3 max-w-md text-sm leading-relaxed text-white/60">
              Power your brand with our complete infrastructure. Custom branding, payment processing,
              vault storage, and worldwide fulfillment.
            </p>
            <div className="mt-6 flex gap-8">
              <div>
                <div className="font-heading text-2xl font-bold text-white">520K+</div>
                <div className="text-[11px] uppercase tracking-wide text-white/45">Units sold</div>
              </div>
              <div>
                <div className="font-heading text-2xl font-bold text-white">5+</div>
                <div className="text-[11px] uppercase tracking-wide text-white/45">Brand partners</div>
              </div>
            </div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-neutral-950 p-6">
            <p className="mb-3 text-[11px] font-medium uppercase tracking-widest text-white/40">Featured Partner</p>
            <h3 className="font-heading text-xl font-bold text-white">Zardo Cards</h3>
            <p className="mt-2 text-[13px] leading-relaxed text-white/55">
              One of the largest online Pokemon stores, now offering digital pack breaks powered by
              Phygitals.
            </p>
          </div>
        </div>
      ...
      <section className="mb-16">
        <Reveal>
          <SectionHeading title="Vault & Security" sub="Top-tier insured US facilities, managed by industry professionals." />
        </Reveal>
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-white/60">
          Open packs, build your collection, and trade globally — or create your own branded
        ...
        <p className="mt-5 text-[13px] text-white/45">
          Reach out to{" "}
          <a href="mailto:hello@phygitals.com" className="text-white/70 underline-offset-2 hover:underline">
            hello@phygitals.com
          </a>
        </p>
      </Reveal>
