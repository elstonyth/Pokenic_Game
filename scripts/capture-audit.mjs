...
const triage = JSON.parse(readFileSync('docs/research/audit/triage.json', 'utf8'));

// width set: [w, fullPage?] — 4K viewport-only to cap token cost
const WIDTHS = [[390, true], [1440, true], [3840, false]];

const clonePaths = triage.rows.filter(r => r.clone === 200).map(r => r.path);
if (!clonePaths.includes('/claw/pokemon-mythic')) clonePaths.push('/claw/pokemon-mythic');
const livePaths = triage.rows.filter(r => r.bucket === 'diffable').map(r => r.path);
  ...
  await page.evaluate(async () => {
    await new Promise(res => {
      ...
      const t = setInterval(() => {
        ...
        if (total >= document.body.scrollHeight + 1200) { clearInterval(t); res(); }
      }, 80);
    });
  }).catch(() => {});
...
async function runJob(job) {
  ...
  try {
    await page.goto(job.base + job.path, { waitUntil: 'networkidle', timeout: 45000 }).catch(e => rec.errors.push('goto:' + e.message.slice(0, 60)));
    ...
    for (const [w, full] of WIDTHS) {
      ...
      await page.screenshot({ path: file, fullPage: full }).catch(e => rec.errors.push(`shot${w}:` + e.message.slice(0, 50)));
      rec.files[w] = file;
    }
  } catch (e) {
  ...
  manifest.push(rec);
  console.log(`${job.site.padEnd(5)} ${job.path.padEnd(34)} ${rec.errors.length ? 'ERR ' + rec.errors.join('|') : 'ok'}`);
}
...
await browser.close();
writeFileSync('docs/research/audit/manifest.json', JSON.stringify(manifest, null, 2));
