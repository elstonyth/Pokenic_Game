...
if (!EMAIL || !PASSWORD) {
  console.warn(
    'PW_CUSTOMER_EMAIL/PW_CUSTOMER_PASSWORD not set — skipping spin-path + spoiler check (reel-fit captures only).',
  );
}
...
try {
  for (const [label, viewport, reducedMotion] of [
    ...
    if (await add.isVisible().catch(() => false)) {
      ...
      const remove = page.getByRole('button', { name: 'Remove a reel' });
