...
> **This repo already wires its hooks** in `.claude/settings.json` +
> `.claude/hooks/` (typecheck-on-edit + a Stop build-verify). The examples below
> use `pnpm` illustratively — **this repo uses `npm` (root) + `corepack yarn`
> (backend)**. Treat the rest of this file as reference patterns, not as hooks to
> add (don't duplicate the typecheck/build hooks).
