...
const lb = await (await fetch(`${API}/store/leaderboard?period=weekly`, {
...
ok("leaderboard_live_top_name", topName && lbHtml.includes(topName), `expected "${topName}"`);
ok("leaderboard_not_mock", !lbHtml.includes("FightingProdigy3098"));
ok("leaderboard_no_email", !/@pokenic\.local|@[\w.-]+\.(com|local)\b/i.test(lbHtml.replace(/elstonyth@outlook\.com/g, "")));
await page.screenshot({ path: `${OUT}/06-leaderboard-live.png`, fullPage: true });

// tab switch -> All Time still renders a table
await page.getByRole("tab", { name: /All Time/i }).click().catch(() => {});
await page.waitForTimeout(500);
const allTimeRows = await page.locator("table tbody tr").count().catch(() => 0);
...
const liveOnlyPack = /Mythic Pack|Legend Pack|Platinum Pack|Pro Pack|Starter Pack|Rookie Pack/.test(feedText);
ok("home_feed_rendered", /Recent Pulls/i.test(feedText));
ok("home_feed_has_cards", feedText.split("\n").filter((l) => /Pack/.test(l)).length >= 3, "pack labels");
