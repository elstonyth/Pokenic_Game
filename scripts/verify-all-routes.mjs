...
const ROUTES = ["/", "/claw", "/claw/pokemon-mythic", "/marketplace", "/leaderboard", "/pack-party", "/how-it-works"];
...
for (const route of ROUTES) {
  ...
  for (const w of WIDTHS) {
    ...
    try {
      ...
      const r = await page.evaluate(() => {
        ...
        const broken = [...document.querySelectorAll("img")].filter((i) => i.complete && i.naturalWidth === 0 && (i.currentSrc || i.src)).length;
        return { over, broken };
      });
  ...
  row.status = row.overflow.length === 0 && row.brokenImgs === 0 ? "OK" : "ISSUE";
  report.push(row);
}
await browser.close();
console.log(JSON.stringify(report, null, 2));
const allOk = report.every((r) => r.status === "OK");
console.log("VERDICT:", allOk ? "PASS — no overflow / broken images on any route" : "ISSUES found");
