...
let livePaths = triage.rows.filter(r => r.bucket === 'diffable').map(r => r.path);
...
async function autoScroll(page) {
  await page.evaluate(async () => {
    await new Promise(res => {
      let t = 0; const s = 600;
      const i = setInterval(() => { window.scrollBy(0, s); t += s; if (t >= document.body.scrollHeight + 1200) { clearInterval(i); res(); } }, 80);
    });
  }).catch(() => {});
...
async function run(path) {
  const dir = `${OUT}/${slug(path)}`; mkdirSync(dir, { recursive: true });
  ...
  try {
    await page.goto(LIVE + path, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(e => rec.errors.push('goto:' + e.message.slice(0, 50)));
    ...
    for (const [w, full] of WIDTHS) {
      ...
      await page.screenshot({ path: file, fullPage: full }).catch(() => rec.errors.push('shot' + w));
      rec.files[w] = file;
    }
  } catch (e) {
  ...
  manifest.push(rec);
  console.log(`live ${path.padEnd(30)} ${rec.errors.length ? 'ERR ' + rec.errors.join('|') : 'ok'}`);
}

const q = [...livePaths];
async function worker() { while (q.length) await run(q.shift()); }
await Promise.all([worker(), worker()]);
await browser.close();
writeFileSync('docs/research/audit/manifest-live.json', JSON.stringify(manifest, null, 2));
