...
for (const slug of slugs) {
  ...
  await img.screenshot({ path: `${OUT}/4k_${tag}.png` }).catch(async () => { await page.screenshot({ path: `${OUT}/4k_${tag}.png` }); });
  console.log(`${slug}: machine box=${box ? Math.round(box.width) + "x" + Math.round(box.height) : "none"}`);
  await ctx.close();
}
await browser.close();
