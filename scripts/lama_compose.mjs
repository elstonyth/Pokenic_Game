...
const results = await page.evaluate(
  async ({ BASES, orig, outp, masks, OVERRIDES, WORD, FSF, BASEF }) => {
    const load = (s) =>
      new Promise((ok, no) => {
        const im = new Image();
        im.onload = () => ok(im);
    ...
    for (const base of BASES) {
      ...
      const wm = idx
        .map(([x, y]) => {
          ...
          const r = od[p],
            g = od[p + 1],
            b = od[p + 2];
          return { r, g, b, d: Math.hypot(r - bg[0], g - bg[1], b - bg[2]) };
        })
        .sort((a, b) => b.d - a.d);
      ...
      const medX = xsAll.length
        ? xsAll[Math.floor(xsAll.length / 2)]
        : (minX + maxX) / 2;
      ...
      if (ov.lama) {
        ...
        for (let ch = 0; ch < 3; ch++) {
          ...
          for (let it = 0; it < 500; it++) {
            // Gauss-Seidel in-place (fast convergence)
            for (let yy = 0; yy < bh; yy++)
              for (let xx = 0; xx < bw; xx++) {
                const i = yy * bw + xx;
                if (!isM[i]) continue;
                const l = xx > 0 ? f[i - 1] : f[i + 1],
                  r = xx < bw - 1 ? f[i + 1] : f[i - 1];
                const u = yy > 0 ? f[i - bw] : f[i + bw],
                  d = yy < bh - 1 ? f[i + bw] : f[i - bw];
                f[i] = (l + r + u + d) * 0.25;
              }
          }
          for (let yy = 0; yy < bh; yy++)
      ...
      if (glow) {
        ctx.shadowColor = col;
        ctx.shadowBlur = Math.round(fs * (ov.blur ?? 0.14));
      ...
      out[base] = {
        ...
        color,
        bg,
        cx: +((cx / W) * 100).toFixed(1),
        baseY: +((baseY / H) * 100).toFixed(1),
        bboxH,
        glow,
      };
