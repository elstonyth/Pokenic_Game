...
const rotY = (page) =>
  page.evaluate(() => {
    ...
    return tr.startsWith("matrix3d") ? (Math.atan2(m[8], m[0]) * 180) / Math.PI : 0;
  });
...
try {
  ...
  for (const x of [740, 700, 640, 560, 470]) { await page.mouse.move(x, 450); await page.waitForTimeout(12); } // fast swipe
  ...
  pass("fling keeps moving after release (no dead stop)", Math.abs(shortly - atRelease) > 8, `release ${atRelease?.toFixed(1)}° -> +120ms ${shortly?.toFixed(1)}°`);
  pass("fling carries past the nearest slot (momentum)", carried > 35, `travelled ${carried.toFixed(0)}° after release`);
  pass("fling still settles on a 60° slot", snapErr < 2, `settled ${settled?.toFixed(1)}°`);
  ...
  pass("shuffle still spinning at +500ms (long decel)", Math.abs(sMid - s0) > 60 && Math.abs(sEnd - sMid) > 5, `${s0?.toFixed(0)}° -> ${sMid?.toFixed(0)}° -> ${sEnd?.toFixed(0)}°`);
  pass("shuffle settled by ~1.5s", Math.abs(sEnd2 - sEnd) < 3, `${sEnd?.toFixed(1)}° -> ${sEnd2?.toFixed(1)}°`);
  ...
  const card = await page.evaluate(() => {
    ...
    const img = [...dlg.querySelectorAll("img")].find((i) => i.src.includes("/cdn/cards/"));
    ...
    for (let el = img.parentElement; el && el !== dlg; el = el.parentElement) {
      ...
      if (m && m.length >= 3 && m[0] > 200 && m[1] > 200 && m[2] > 200) { chrome = bg; break; }
    }
    const btn = [...dlg.querySelectorAll("button")].find((b) => /^Continue$/.test(b.textContent.trim()));
    return { found: true, imgW: Math.round(r.width), imgH: Math.round(r.height), chrome, btnW: btn ? Math.round(btn.getBoundingClientRect().width) : null };
  });
  pass("card stage reached + card img rendered", card.found, JSON.stringify(card));
  pass("no white holder chrome around the card", card.found && !card.chrome, card.chrome ?? "raw image");
  pass("card sized like live (~560px tall at 1440)", card.found && card.imgH >= 520 && card.imgH <= 600, `img ${card.imgW}x${card.imgH} (live 330x569)`);
  pass("Continue is 300px wide (live)", card.btnW === 300, `btn ${card.btnW}px`);
